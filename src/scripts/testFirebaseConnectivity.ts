...
const testFirebaseConfig = () => {
  ...
  try {
    ...
    const missingKeys = requiredKeys.filter(
      key => !(key in firebaseConfig) || !firebaseConfig[key as keyof typeof firebaseConfig]
    );
    ...
    try {
      ...
      if (user) {
        printResult(
          'Anonymous Auth',
          true,
          `Signed in anonymously with UID: ${user.uid.substring(0, 8)}...`
        );
...
const testFirestore = async () => {
  ...
  try {
    ...
    if (
      retrievedDoc &&
      'timestamp' in retrievedDoc &&
      retrievedDoc.timestamp === testDoc.timestamp
    ) {
    ...
    // Test querying
    const querySnapshot = await getDocs(query(collection(db, collectionName), limit(10)));

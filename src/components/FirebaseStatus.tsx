...
const FirebaseStatus: React.FC = () => {
  const [overallStatus, setOverallStatus] = useState<'checking' | 'connected' | 'error'>(
    'checking'
  );
  ...
  useEffect(() => {
    ...
    const validateConfig = () => {
      ...
      const missingKeys = requiredKeys.filter(
        key => !(key in config) || !config[key as keyof typeof firebaseConfig]
      );

      const configValues = Object.entries(config).map(
        ([key, value]) => `${key}: ${value ? String(value).substring(0, 5) + '...' : 'undefined'}`
      );
      ...
      return (
        ...
        >
          ...
          <ol style={{ paddingLeft: '1.5rem', marginBottom: 0 }}>
            <li>
              Check if you've created a Firebase project in the{' '}
              <a
            ...
            <li>
              Verify your environment variables in the <code>.env</code> file match your Firebase
            ...
            <li>
              For help, see <code>docs/FIREBASE_SETUP_GUIDE.md</code>
            </li>
          </ol>
        </div>
      );

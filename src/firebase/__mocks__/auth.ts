...
export const signIn = jest.fn<(email: string, password: string) => Promise<UserCredential>>()
  .mockImplementation(() => Promise.resolve(mockUserCredential));
export const signUp = jest.fn<(email: string, password: string) => Promise<UserCredential>>()
  .mockImplementation(() => Promise.resolve(mockUserCredential));
export const logout = jest.fn<() => Promise<void>>()
  .mockImplementation(() => Promise.resolve());
export const resetPassword = jest.fn<(email: string) => Promise<void>>()
  .mockImplementation(() => Promise.resolve());

// Create a mock for auth state change that immediately calls the callback
export const onAuthChange = jest.fn<
  (callback: (user: User) => void) => () => void
